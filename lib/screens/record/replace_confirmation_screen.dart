...
class ReplaceConfirmationScreen extends StatefulWidget {
  final ChemicalMaterial material;
  final DateTime date;
  final double amount;
  final String userLmaId;
  const ReplaceConfirmationScreen({
    Key? key,
  ...
  @override
  State<ReplaceConfirmationScreen> createState() =>
      _ReplaceConfirmationScreenState();
}

class _ReplaceConfirmationScreenState extends State<ReplaceConfirmationScreen> {
  late int percent = 0;
  late String message = '';
  ...
  void _sendMaterial() async {
    ...
    if (widget.amount > widget.material.initialAmount) {
      await db.collection('cabinet').doc(widget.material.id).update({
        'initialAmount': widget.material.currentAmount + widget.amount,
      });
    }
